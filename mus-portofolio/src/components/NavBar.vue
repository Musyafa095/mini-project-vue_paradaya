<template>
  ...
  <!-- sun icon -->
  <svg
  v-if=" !isNightTheme" class="swap-off h-10 w-10 fill-current text-orange-600"
  ...
  <!-- moon icon -->
  <svg
    v-if="isNightTheme" class="swap-on h-10 w-10 fill-current text-teal-500"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24">
    <path
...
const isNightTheme = computed({
  get() {
    return ThemeStore.theme === 'night'; 
  },
  set(value) {
    ThemeStore.theme = value ? 'night' : 'nord'; 
